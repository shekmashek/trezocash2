...
export const useCashflowChartData = (dataState, uiState, periods) => {
    ...
    return useMemo(() => {
        if (!periods || periods.length === 0 || !dataState.settings) {
            return { labels: [], inflows: [], outflows: [], actualBalance: [], projectedBalance: [], periodPositions: [] };
        }
...
export const useDashboardKpis = (dataState, uiState) => {
    const { actualTransactions, budgetEntries } = useActiveProjectData(dataState, uiState);
    const { settings, allCashAccounts, allActuals, consolidatedViews, loans } = dataState;
    const balances = useAccountBalances(allCashAccounts, allActuals, uiState.activeProjectId, uiState.isConsolidated, uiState.isCustomConsolidated, consolidatedViews);

    return useMemo(() => {
        if (!settings) {
             return { totalActionableBalance: 0, totalOverduePayables: 0, totalOverdueReceivables: 0, overdueItems: [], totalSavings: 0, totalProvisions: 0, totalBorrowings: 0, totalLendings: 0 };
        }
        const today = getTodayInTimezone(settings.timezoneOffset);
        ...
        const calculateRemainingLoanBalance = (loan) => {
            const repaymentEntry = budgetEntries.find(e => e.loanId === loan.id && e.type !== (loan.type === 'borrowing' ? 'revenu' : 'depense'));
            if (!repaymentEntry) return loan.principal;

            const repaymentActuals = actualTransactions.filter(a => a.budgetId === repaymentEntry.id);
            const totalPaid = repaymentActuals.reduce((sum, actual) => sum + (actual.payments || []).reduce((pSum, p) => pSum + p.paidAmount, 0), 0);
            
            if (!loan.monthlyPayment || !loan.term || loan.monthlyPayment <= 0 || loan.term <= 0) return Math.max(0, loan.principal - totalPaid);
            
            const totalLoanCost = loan.monthlyPayment * loan.term;
            if (totalLoanCost < loan.principal) return Math.max(0, loan.principal - totalPaid);
        ...
        const totalBorrowings = (loans || []).filter(l => l.type === 'borrowing').reduce((sum, l) => sum + calculateRemainingLoanBalance(l), 0);
        const totalLendings = (loans || []).filter(l => l.type === 'lending').reduce((sum, l) => sum + calculateRemainingLoanBalance(l), 0);

        return { totalActionableBalance, totalOverduePayables, totalOverdueReceivables, overdueItems, totalSavings, totalProvisions, totalBorrowings, totalLendings };
    }, [actualTransactions, budgetEntries, settings, balances, loans]);
};
...
export const useScheduleData = (actualTransactions, settings) => {
    return useMemo(() => {
        ...
        (actualTransactions || []).forEach(actual => {
            ...
            const isSettled = ['paid', 'received', 'written_off'].includes(actual.status);
...
export const useLoanSummary = (dataState, uiState) => {
    ...
    return useMemo(() => {
        ...
        const processLoans = (entryType) => {
            ...
            const mainCatName = isBorrowing ? 'Emprunt ( Remboursement d\')' : 'Prêts (Remboursement de)';
