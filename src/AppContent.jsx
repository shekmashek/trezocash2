...
function AppContent() {
  ...
  const { uiState, uiDispatch } = useUI();
  const { session, profile, projects, tiers, templates } = dataState;
    ...
    if (session && !profile) {
      const fetchInitialData = async () => {
        ...
        try {
          ...
          if (projectsError) {
              const userMessage = `Erreur de chargement des projets: ${projectsError.message}. Si le problème persiste, veuillez contacter le support.`;
              uiDispatch({ type: 'ADD_TOAST', payload: { message: userMessage, type: 'error', duration: 20000 } });
          ...
          const [
            ...
            supabase.from('consolidated_views').select('*'), supabase.from('collaborators').select('id, owner_id, user_id, email, status, project_ids, permissions'), supabase.from('comments').select('*'),
          ...
          const consolidatedViews = (consolidatedViewsRes.data || []).map(v => ({ id: v.id, name: v.name, project_ids: v.project_ids, is_archived: v.is_archived }));
          ...
          if (isNewSignup && projectsData.length === 0) {
            const demoProjectPayload = {
                projectName: 'Mon mariage - démo',
                projectDescription: 'Un projet de démonstration pour planifier le budget d\'un mariage.',
                projectStartDate: new Date().toISOString().split('T')[0],
                isEndDateIndefinite: false,
                projectEndDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)).toISOString().split('T')[0],
                templateId: 'demo-mariage',
                startOption: 'populated',
                projectType: 'event',
                projectClass: 'event'
            };
            await initializeProject({ dataDispatch, uiDispatch }, demoProjectPayload, session.user, tiers, templates);
          ...
          uiDispatch({ type: 'ADD_TOAST', payload: { message: `Erreur: ${error.message}`, type: 'error', duration: 10000 } });
        } finally {
