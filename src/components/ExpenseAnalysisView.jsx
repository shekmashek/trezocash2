...
import { PieChart, ChevronLeft, ChevronRight, ArrowLeft, Folder, User, ChevronDown } from 'lucide-react';
...
const ExpenseAnalysisView = ({ isFocusMode = false, rangeStart: rangeStartProp, rangeEnd: rangeEndProp, analysisType: analysisTypeProp, analysisMode: analysisModeProp, setAnalysisMode: setAnalysisModeProp }) => {
  ...
  const { projects, categories, allActuals, allEntries, consolidatedViews } = dataState;
  const { activeProjectId, onboardingCompletedProjects } = uiState;
  ...
  const analysisType = analysisTypeProp || 'expense';
  ...
  const onboardingCompleted = useMemo(() => 
      onboardingCompletedProjects?.includes(activeProjectId), 
  [onboardingCompletedProjects, activeProjectId]);

  const projectHasEntries = useMemo(() => {
      if (!activeProjectId || isConsolidated || isCustomConsolidated) return true;
      const entries = allEntries[activeProjectId];
      return entries && entries.length > 0;
  }, [activeProjectId, allEntries, isConsolidated, isCustomConsolidated]);

  const projectHasConfiguredAccounts = useMemo(() => {
      if (isConsolidated || isCustomConsolidated || !activeProjectId) return true;
      const accounts = dataState.allCashAccounts[activeProjectId];
      return accounts && accounts.length > 0;
  }, [activeProjectId, dataState.allCashAccounts, isConsolidated, isCustomConsolidated]);

  const showOnboardingButton = projectHasEntries && projectHasConfiguredAccounts && !onboardingCompleted;

  const handleCompleteOnboarding = () => {
    uiDispatch({ type: 'COMPLETE_PROJECT_ONBOARDING', payload: activeProjectId });
  ...
  const { rangeStart, rangeEnd } = useMemo(() => {
    if (rangeStartProp && rangeEndProp) {
        return { rangeStart: new Date(rangeStartProp), rangeEnd: new Date(rangeEndProp) };
    }
  ...
  }, [rangeStartProp, rangeEndProp, localTimeUnit, localPeriodOffset, dataState.settings.timezoneOffset]);
  ...
  const analysisPeriodName = useMemo(() => {
    ...
    switch (localTimeUnit) {
        case 'month': return rangeStart.toLocaleString('fr-FR', { month: 'long', year: 'numeric' });
        case 'bimonthly': const startMonthB = rangeStart.toLocaleString('fr-FR', { month: 'short' }); const endMonthB = new Date(year, month + 1, 1).toLocaleString('fr-FR', { month: 'short' }); return `Bimestre ${startMonthB}-${endMonthB} ${year}`;
        case 'quarterly': const quarter = Math.floor(month / 3) + 1; return `Trimestre ${quarter} ${year}`;
        case 'semiannually': const semester = Math.floor(month / 6) + 1; return `Semestre ${semester} ${year}`;
        case 'annually': return `Année ${year}`;
        default: return '';
    }
  ...
  const projectActuals = useMemo(() => {
    ...
    if (isConsolidated) {
        ...
        relevant = view.project_ids.flatMap(id => allActuals[id] || []);
    } else {
  ...
  const projectEntries = useMemo(() => {
    ...
    if (isConsolidated) {
        ...
        const view = consolidatedViews.find(v => v.id === viewId);
        if (!view || !view.project_ids) return [];
        relevant = view.project_ids.flatMap(id => allEntries[id] || []);
    } else {
  ...
  const expandedProjectEntries = useMemo(() => expandVatEntries(projectEntries, categories), [projectEntries, categories]);
  ...
  const getChartOptions = () => {
    ...
    if (visibleData.budget) {
        series.push({
            name: `Budget: ${formatCurrency(totalBudget, dataState.settings)}`,
            type: 'bar',
            data: budgetData,
            ...
            label: {
                ...
                formatter: (params) => {
                    ...
                    return `${formatCurrency(params.value, dataState.settings)} (${percentage.toFixed(0)}%)`;
                },
    ...
    if (visibleData.actual) {
        series.push({
            name: `Réel: ${formatCurrency(totalActual, dataState.settings)}`,
            type: 'bar',
            data: actualData,
    ...
    return {
        title: { text: `Analyse par Catégorie - ${analysisType === 'expense' ? 'Sorties' : 'Entrées'}`, left: 'center', top: 0, textStyle: { fontSize: 16, fontWeight: '600', color: '#475569' } },
        tooltip: {
            trigger: 'axis',
            ...
            formatter: (params) => {
                ...
                params.slice().reverse().forEach(param => {
                    ...
                    tooltip += `${param.marker} ${seriesName.split(':')[0]}: <strong>${formatCurrency(param.value, dataState.settings)}</strong> (${percentage.toFixed(1)}%)<br/>`;
                });
                return tooltip;
            }
        },
        ...
        xAxis: { type: 'value', axisLabel: { formatter: (value) => formatCurrency(value, { ...dataState.settings, displayUnit: 'standard' }) } },
        yAxis: { type: 'category', data: categories, axisLabel: { interval: 0, rotate: 0 } },
        series: series
    };
  };
  ...
  const getSubCategoryDrillDownChartOptions = () => {
    ...
    return {
        ...
        tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: (params) => { const param = params[0]; const percentage = total > 0 ? (param.value / total) * 100 : 0; return `<strong>${param.name}</strong><br/>${param.marker} ${param.seriesName}: <strong>${formatCurrency(param.value, dataState.settings)}</strong> (${percentage.toFixed(1)}%)`; } },
        grid: { left: '3%', right: '10%', bottom: '3%', containLabel: true },
        xAxis: { type: 'value', axisLabel: { formatter: (value) => formatCurrency(value, { ...dataState.settings, displayUnit: 'standard' }) } },
        yAxis: { type: 'category', data: data.map(d => d.name), axisLabel: { interval: 0 } },
        series: [{ name: drillDownState.dataType === 'actual' ? 'Réel' : 'Budget', type: 'bar', data: data.map(d => d.value), itemStyle: { color: barColor, borderRadius: [0, 5, 5, 0] }, label: { show: true, position: 'right', formatter: (params) => { if (params.value <= 0) return ''; const percentage = total > 0 ? (params.value / total) * 100 : 0; return `${formatCurrency(params.value, dataState.settings)} (${percentage.toFixed(0)}%)`; }, color: labelColor } }]
    };
  };
  ...
  const getProjectChartOptions = () => {
    ...
    const series = [];
    if (visibleData.budget) { series.push({ name: 'Budget', type: 'bar', data: budgetData, itemStyle: { color: chartColors.budget, borderRadius: [0, 5, 5, 0] }, label: { show: true, position: 'right', color: chartColors.budgetLabel } }); }
    if (visibleData.actual) { series.push({ name: 'Réel', type: 'bar', data: actualData, itemStyle: { color: chartColors.actual, borderRadius: [0, 5, 5, 0] }, label: { show: true, position: 'right', color: chartColors.actualLabel } }); }
    return {
  ...
  const quickPeriodOptions = [
    { id: 'month', label: 'Mois' }, { id: 'bimester', label: 'Bimestre' }, { id: 'quarter', label: 'Trimestre' },
    { id: 'semester', label: 'Semestre' }, { id: 'year', label: 'Année' },
  ];
  ...
  return (
    <div className={isFocusMode ? "h-full flex flex-col" : ""}>
      {!isFocusMode && (
        <div className="mb-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
                                    ...
                                    <motion.div initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} className="absolute left-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border z-20">
                                        <ul className="p-1">
                                            {quickPeriodOptions.map(option => (
                                                <li key={option.id}><button onClick={() => { handleQuickPeriodSelect(option.id); setIsPeriodMenuOpen(false); }} className={`w-full text-left px-3 py-1.5 text-sm rounded-md ${localActiveQuickSelect === option.id ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-gray-100'}`}>{option.label}</button></li>
                                            ))}
                                        </ul>
                                    </motion.div>
                                )}
                ...
                <div className="flex items-center gap-4">
                    <div className="relative" ref={analysisModeMenuRef}>
                        <button onClick={() => setIsAnalysisModeMenuOpen(p => !p)} className="flex items-center gap-2 px-3 h-9 rounded-md bg-gray-200 text-gray-700 font-semibold text-sm hover:bg-gray-300 transition-colors">
                        ...
                        <AnimatePresence>
                            {isAnalysisModeMenuOpen && (
                                <motion.div initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }} className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-lg border z-20">
                                    <ul className="p-1">
                                        {analysisModeOptions.map(option => (
                                            <li key={option.id}><button onClick={() => { setAnalysisMode(option.id); setIsAnalysisModeMenuOpen(false); }} className={`w-full text-left px-3 py-1.5 text-sm rounded-md ${analysisMode === option.id ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-gray-100'}`}>{option.label}</button></li>
                                        ))}
                                    </ul>
                                </motion.div>
                            )}
                    ...
                    <div className="flex items-center gap-1 bg-gray-200 p-1 rounded-lg">
                        <button onClick={() => setVisibleData(p => ({ ...p, budget: !p.budget }))} className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors ${visibleData.budget ? 'bg-white shadow text-blue-600' : 'text-gray-600 hover:bg-gray-300'}`}>Budget</button>
                        <button onClick={() => setVisibleData(p => ({ ...p, actual: !p.actual }))} className={`px-3 py-1.5 text-sm font-semibold rounded-md transition-colors ${visibleData.actual ? 'bg-white shadow text-blue-600' : 'text-gray-600 hover:bg-gray-300'}`}>Réel</button>
                    </div>
                </div>
            </div>
        </div>
