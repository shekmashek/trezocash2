...
const PaymentModal = ({ isOpen, onClose, actualToPay }) => {
  ...
  const { totalDue, totalPaid, remainingAmount, type } = useMemo(() => {
    if (!actualToPay) {
      return { totalDue: 0, totalPaid: 0, remainingAmount: 0, type: 'payable' };
    }
    const due = actualToPay.amount;
    const paid = (actualToPay.payments || []).reduce((sum, p) => sum + p.paidAmount, 0);
    const remaining = due - paid;
    return { totalDue: due, totalPaid: paid, remainingAmount: remaining, type: actualToPay.type };
  }, [actualToPay]);

  const projectForPayment = useMemo(() => {
    const projectId = actualToPay?.projectId || activeProjectId;
    return projects.find(p => p.id === projectId);
  }, [projects, activeProjectId, actualToPay]);

  const userCashAccounts = useMemo(() => {
    const projectId = actualToPay?.projectId;
    if (projectId) {
        return allCashAccounts[projectId] || [];
    }
    // Fallback for older data structures or when projectId is not available on actualToPay
    return allCashAccounts[activeProjectId] || [];
  }, [allCashAccounts, activeProjectId, actualToPay]);

  useEffect(() => {
    if (isOpen && actualToPay) {
      setPaymentDate(new Date().toISOString().split('T')[0]);
      setPaidAmount(remainingAmount > 0 ? remainingAmount.toFixed(2) : '');
      const activeAccounts = userCashAccounts.filter(acc => !acc.isClosed);
      setCashAccount(activeAccounts.length > 0 ? activeAccounts[0].id : '');
    }
  }, [isOpen, remainingAmount, userCashAccounts, actualToPay]);
  ...
  if (!isOpen || !actualToPay) return null;

  const titleText = type === 'payable' ? 'Enregistrer un Paiement' : 'Enregistrer un Encaissement';
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-lg w-full">
        <div className="flex items-center justify-between p-4 border-b"><h2 className="text-xl font-semibold text-gray-900">{titleText}</h2><button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X className="w-6 h-6" /></button></div>
        <div className="p-6">
          ...
          <div className="space-y-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-2">{dateLabel}</label><input type="date" value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg" required min={projectForPayment?.startDate} /></div>
