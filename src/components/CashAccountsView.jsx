...
import { commonCurrencies } from '../utils/currencies';
...
const CashAccountsView = () => {
  ...
  const handleStartEdit = (account) => {
    setEditingAccount({
      ...
      currency: account.currency || activeProject?.currency || settings.currency,
    });
  };
          ...
          {accountBalances.length > 0 ? (
            <ul className="divide-y divide-gray-200">
              {accountBalances.map(account => {
                const accountCurrencySettings = { ...settings, currency: account.currency };
                return (
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select
                                ...
                                onChange={(e) => setEditingAccount(d => ({ ...d, currency: e.target.value }))}
                    ...
                    <div className="group flex items-center justify-between">
                      <div className="flex-grow">
                        ...
                        <div className="text-sm text-gray-500 ml-8 mt-1">
                          Solde initial: <span className="font-semibold">{formatCurrency(account.initialBalance || 0, accountCurrencySettings)}</span> le {new Date(account.initialBalanceDate).toLocaleDateString('fr-FR')}
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className="font-bold text-lg text-gray-800">{formatCurrency(account.balance, accountCurrencySettings)}</p>
