...
const FocusView = () => {
    ...
    const { settings, tiers } = dataState;
    ...
    const handleNewBudgetEntry = () => {
        if (isConsolidated || isCustomConsolidated) return;
        const onSave = (entryData) => {
            saveEntry({dataDispatch, uiDispatch, dataState}, {
                ...
                tiers: tiers,
                cashAccounts: dataState.allCashAccounts[activeProjectId] || [],
    ...
    const focusNewMenuItems = [
        { label: 'Budget prévisionnel', icon: Plus, action: handleNewBudgetEntry, disabled: isConsolidated || isCustomConsolidated, tooltip: (isConsolidated || isCustomConsolidated) ? "Non disponible en vue consolidée" : "Ajouter une nouvelle entrée ou sortie prévisionnelle" },
        { label: 'Entrée reçue', icon: TrendingUp, action: () => uiDispatch({ type: 'OPEN_DIRECT_PAYMENT_MODAL', payload: 'receivable' }), disabled: isConsolidated || isCustomConsolidated, tooltip: (isConsolidated || isCustomConsolidated) ? "Non disponible en vue consolidée" : "Encaisser directement des entrées" },
        { label: 'Sortie payée', icon: TrendingDown, action: () => uiDispatch({ type: 'OPEN_DIRECT_PAYMENT_MODAL', payload: 'payable' }), disabled: isConsolidated || isCustomConsolidated, tooltip: (isConsolidated || isCustomConsolidated) ? "Non disponible en vue consolidée" : "Payer directement des sorties" }
    ];
        ...
        <div className="h-screen w-screen bg-background p-4 flex flex-col">
            <div className="flex justify-between items-start mb-4 flex-shrink-0">
                ...
                <div className="flex items-center gap-2">
                    <div className="relative" ref={newMenuRef}><button onClick={() => setIsNewMenuOpen(p => !p)} disabled={isConsolidated || isCustomConsolidated} className="flex items-center justify-center h-9 w-9 rounded-full bg-blue-600 text-white hover:bg-blue-700 transition-colors shadow-md disabled:bg-gray-400 disabled:cursor-not-allowed" title="Créer"><Plus className="w-5 h-5" /></button>
