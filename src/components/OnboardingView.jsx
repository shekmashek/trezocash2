...
import { ArrowRight, ArrowLeft, Sparkles, Loader, Search, Star, Users, LayoutTemplate, FilePlus, Briefcase, Home, PartyPopper } from 'lucide-react';
...
import { eventTemplates } from '../utils/eventTemplates';
...
const OnboardingView = () => {
  ...
  const { onboardingContext } = uiState;
  ...
  useEffect(() => {
    if (onboardingContext?.projectClass) {
      const projectClass = onboardingContext.projectClass;
      const projectType = projectClass === 'event' ? 'event' : 'business';
      setData(prev => ({ ...prev, projectClass, projectType }));
      setStep(1); // Skip the type selection step
    }
  }, [onboardingContext]);
  ...
  const allOfficialTemplates = useMemo(() => {
    ...
    if (data.projectClass === 'event') {
      return [blankTemplate, ...eventTemplates];
    }
        ...
        return (
            <div className="text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
                    <button onClick={() => { setData(prev => ({ ...prev, projectClass: 'treasury', projectType: 'business' })); handleNext(); }} className="p-8 border-2 rounded-lg text-left transition-all hover:border-blue-500 hover:bg-blue-50 text-gray-800">
                        <Briefcase className="w-8 h-8 text-blue-600 mb-4" />
                        <h3 className="font-semibold text-lg">Professionnel</h3>
                        <p className="text-sm text-gray-600 mt-1">Pour piloter la trésorerie de votre activité.</p>
                    </button>
                    <button onClick={() => { setData(prev => ({ ...prev, projectClass: 'treasury', projectType: 'household' })); handleNext(); }} className="p-8 border-2 rounded-lg text-left transition-all hover:border-green-500 hover:bg-green-50 text-gray-800">
                        <Home className="w-8 h-8 text-green-600 mb-4" />
                        <h3 className="font-semibold text-lg">Ménage</h3>
                        <p className="text-sm text-gray-600 mt-1">Pour gérer votre budget personnel ou familial.</p>
                    </button>
                    <button onClick={() => { setData(prev => ({ ...prev, projectClass: 'event', projectType: 'event' })); handleNext(); }} className="p-8 border-2 rounded-lg text-left transition-all hover:border-pink-500 hover:bg-pink-50 text-gray-800">
                        <PartyPopper className="w-8 h-8 text-pink-600 mb-4" />
                        <h3 className="font-semibold text-lg">Événement</h3>
                        <p className="text-sm text-gray-600 mt-1">Pour un projet ponctuel (mariage, voyage...).</p>
                    </button>
                </div>
            </div>
        );
          ...
          <div className="text-center">
            ...
            <div className="space-y-6 max-w-md mx-auto">
              ...
              <div>
                ...
                <textarea value={data.projectDescription} onChange={(e) => setData(prev => ({ ...prev, projectDescription: e.target.value }))} placeholder="Quel est l'objectif de ce projet ?" className="w-full text-base p-2 border-b-2 focus:border-blue-500 outline-none transition bg-transparent" rows="2"></textarea>
              </div>
    ...
    <div className="bg-gray-100 min-h-screen flex flex-col items-center justify-center p-4 antialiased">
      ...
      <div className="w-full max-w-4xl mx-auto bg-white rounded-2xl shadow-xl flex flex-col" style={{ minHeight: '600px' }}>
        ...
        <div className="p-6 bg-gray-50 border-t flex justify-between items-center">
          ...
          {step < steps.length - 1 && (
            <button onClick={handleNext} disabled={isLoading || (step === 1 && !data.projectName.trim()) || (step === 2 && !data.templateId)} className="flex items-center gap-2 px-6 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 font-semibold disabled:bg-gray-400">
