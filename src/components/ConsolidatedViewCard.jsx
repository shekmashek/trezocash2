...
import { Layers, MoreVertical, Edit, Archive, Trash2 } from 'lucide-react';
import { useUI } from '../context/UIContext';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';

const ConsolidatedViewCard = ({ view, onEdit, onArchive, onDelete }) => {
    ...
    const handleCardClick = () => {
        uiDispatch({ type: 'SET_ACTIVE_PROJECT', payload: `consolidated_view_${view.id}` });
    ...
    return (
        <div className="bg-white rounded-lg shadow-md border border-gray-200 flex flex-col justify-between hover:shadow-lg transition-shadow cursor-pointer relative">
            ...
            <div className="px-5 pb-5 mt-auto" onClick={handleCardClick}>
                <div className="h-10">
                    <p className="text-xs text-gray-500">{view.project_ids.length} projets inclus</p>
                </div>
            </div>
        </div>
    );
