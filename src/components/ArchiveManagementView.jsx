...
import { restoreConsolidatedView } from '../context/actions';
import { useUI } from '../context/UIContext';

const ArchiveManagementView = () => {
  const { dataState, dataDispatch } = useData();
  const { uiDispatch } = useUI();
  const { projects, scenarios, consolidatedViews } = dataState;

  const archivedProjects = projects.filter(p => p.isArchived);
  const archivedScenarios = scenarios.filter(s => s.isArchived);
  const archivedConsolidatedViews = consolidatedViews.filter(v => v.is_archived);
  ...
  const handleRestoreView = (viewId) => {
    restoreConsolidatedView({ dataDispatch, uiDispatch }, viewId);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
          ...
          Vues Consolidées Archivées
        </h2>
        {archivedConsolidatedViews.length > 0 ? (
          <ul className="divide-y divide-gray-200">
            {archivedConsolidatedViews.map(view => (
              <li key={view.id} className="py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Layers className="w-5 h-5 text-gray-400" />
                  <span className="font-medium text-gray-700">{view.name}</span>
                </div>
                <button
                  onClick={() => handleRestoreView(view.id)}
                  className="bg-green-100 text-green-700 hover:bg-green-200 px-3 py-1 rounded-md font-medium flex items-center gap-2 text-sm"
                >
                  <ArchiveRestore className="w-4 h-4" />
                  Restaurer
          ...
          <EmptyState
            icon={Archive}
            title="Aucune vue consolidée archivée"
            message="Les vues que vous archivez apparaîtront ici."
          />
        )}
      </div>
