/*
          # [Operation Name]
          Update Invite Collaborator Function

          ## Query Description: [This operation updates the `invite_collaborator` function to handle pending invitations for non-existent users and to accept a budget limit for editor roles. It ensures that an invitation is created even if the user has not signed up yet, and this invitation will be automatically activated upon their registration.]
          ...
          ## Performance Impact:
          - Indexes: [None]
          - Triggers: [This function is used in conjunction with a new trigger on `auth.users`.]
